...
- Name: `agent-harness`
- Purpose: Source, install, verify, and document the shared Claude, Codex, and Cursor harness and its receiving-computer Capsule.
...
- Setup: `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\.agents\tools\Manage-Harness.ps1 -Action EnsureProject -Repository .`
- Test: `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\.agents\tools\Test-HarnessSetup.ps1`
- Lint: `git diff --check`
- Build: `N/A — scripts and documentation`
- End-to-end: `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\.agents\tools\Test-HarnessSetup.ps1`

Record the actual command or observable proof under `TASK.md` → `Verification`.

## Project-specific rules

- Keep the Capsule limited to the shared global harness and approved Obsidian configuration.
- Keep project source and safe project files in each project's Git repository.
- Keep external project data under the project's declared `data-manifest.yaml` adapter.
- Keep the human-facing harness explanation consolidated in `.agents\human-readable\README.md` and its HTML mirror.
