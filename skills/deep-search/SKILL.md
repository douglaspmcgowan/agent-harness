...
- **Dispatch to a background `general-purpose` Agent** (`run_in_background: true`): use when total calls exceed 12, Stage 3 is needed and adds significantly more fetches, or your main context needs to stay free. Encode all three stages into the agent's brief; require incremental Writes.
