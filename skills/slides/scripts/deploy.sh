...
#
# Usage:
#   bash scripts/deploy.sh <path-to-slide-folder-or-html> --yes-publish-public
#
# Examples:
#   bash scripts/deploy.sh ./my-pitch-deck/ --yes-publish-public
#   bash scripts/deploy.sh ./presentation.html --yes-publish-public
#
# What this does:
#   0. PUBLISH-SAFETY GATE — refuses to deploy until you explicitly confirm
#      that the deck may go on the PUBLIC internet (see below).
...
#
# Adapted from html-slides/scripts/deploy.sh. The only behavioral change vs.
# the source is the publish-safety gate (Step 0); the deploy logic is unchanged.
set -euo pipefail
...
# ═══════════════════════════════════════════════════════════════════════════
# STEP 0: PUBLISH-SAFETY GATE  (runs BEFORE anything else)
# ═══════════════════════════════════════════════════════════════════════════
# This skill may be used on sensitive / CUI / pre-decisional material that must
# NEVER be auto-published to the open internet. Vercel deployments are PUBLIC:
# anyone with the URL can view the deck, and the URL is not access-controlled.
#
# We therefore require an EXPLICIT opt-in before deploying. The user must either:
#   • pass the --yes-publish-public flag, OR
#   • answer "y" to the interactive prompt (which DEFAULTS TO NO).
#
# Without confirmation, we print the warning and exit 0 (success, no-op) so that
# an accidental/scripted invocation never publishes anything.
# ═══════════════════════════════════════════════════════════════════════════

# Pull the --yes-publish-public flag out of the args first (so the rest of the
# script sees only the path argument, exactly like the source script).
...
# Restore positional args without the flag (guard against empty array under set -u).
set -- ${POSITIONAL[@]+"${POSITIONAL[@]}"}

echo ""
echo -e "${BOLD}${YELLOW}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${YELLOW}║  ⚠  PUBLISH WARNING — this deploys to the PUBLIC internet   ║${NC}"
echo -e "${BOLD}${YELLOW}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "  Deploying to Vercel makes this deck ${BOLD}publicly viewable${NC} by anyone"
echo -e "  with the URL. The URL is ${BOLD}not${NC} password-protected or access-controlled."
echo ""
echo -e "  ${RED}Do NOT deploy sensitive, CUI, ITAR, pre-decisional, or otherwise${NC}"
echo -e "  ${RED}non-public material this way.${NC}"
echo ""

if [[ "$PUBLISH_CONFIRMED" != "true" ]]; then
    # No flag — fall back to an interactive prompt that DEFAULTS TO NO.
    # If there's no TTY (non-interactive run), read returns empty → treated as "no".
    if [[ -t 0 ]]; then
        printf "  Publish this deck to the public internet? [y/N] "
        read -r REPLY || REPLY=""
        case "$REPLY" in
            [yY]|[yY][eE][sS])
                PUBLISH_CONFIRMED=true
...
if [[ "$PUBLISH_CONFIRMED" != "true" ]]; then
    echo ""
    warn "Publish not confirmed — nothing was deployed."
    info "To deploy, re-run with the explicit flag:"
    echo ""
    echo -e "    ${BOLD}bash scripts/deploy.sh <path> --yes-publish-public${NC}"
    echo ""
    # Exit 0: a refused publish is a safe no-op, not an error.
    exit 0
fi

ok "Public publish confirmed — continuing with deployment."

# ─── Input validation ─────────────────────────────────────

if [[ $# -lt 1 ]]; then
    err "Usage: bash scripts/deploy.sh <path-to-slide-folder-or-html> --yes-publish-public"
    err ""
    err "Examples:"
    err "  bash scripts/deploy.sh ./my-pitch-deck/ --yes-publish-public"
    err "  bash scripts/deploy.sh ./presentation.html --yes-publish-public"
    exit 1
fi
...
if [[ -f "$INPUT" && "$INPUT" == *.html ]]; then
    ...
    #
    # NOTE for slides-ultra: a deck created by new-deck.sh references the skill's
    # shared assets via RELATIVE paths like "../../assets/base.css". Those parent
    # paths are skipped below (the `grep -v '^/'` only drops absolute paths, but a
    # `../../` reference resolves OUTSIDE the deck folder and won't be copied with
    # its structure intact). For reliable single-file deploys, prefer a deck whose
    # assets are inlined or sit in a local ./assets/ folder. The assets/ copy below
...
if ! command -v npx &>/dev/null; then
    ...
    err "  Windows: download the installer from https://nodejs.org"
    exit 1
fi
