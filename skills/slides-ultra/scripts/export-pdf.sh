...
#
# This is a LOCAL, OFFLINE operation — nothing is published or uploaded.
...
#
# Adapted from html-slides/scripts/export-pdf.sh. KEY DIFFERENCE: this skill's
# decks use a `.deck > .slide` structure where the runtime supports a
# `?preview=N` URL param (1-based) that renders a single slide with NO chrome,
# and `#/N` hash deep-links. The source script instead assumed a flat `.slide`
# layout with an `.active` class and a global `window.presentation.goToSlide()`
# API, manually toggling display/opacity and forcing `.reveal` elements visible.
# That manual approach does not match this runtime. Instead we load each slide
# via `?preview=N`, which is exactly the chrome-free single-slide mode the
# runtime already implements — so screenshots come out clean without selector hacks.
set -euo pipefail
...
//
// slides-ultra adaptation: each slide is captured by RELOADING the deck with a
// ?preview=N query param. The runtime's preview-only mode shows exactly one
// slide (1-based index N), hides all chrome (progress bar, notes, overview),
// and disables keyboard/hash handling — so each screenshot is a clean, isolated
// slide with no manual DOM surgery required.
...
const server = createServer((req, res) => {
  // Strip the query string (e.g. ?preview=3) before resolving the file path.
...
// ─── Discover how many slides the deck has ────────────────
...
const baseUrl = `http://localhost:${port}/${HTML_FILE}`;

await page.goto(baseUrl, { waitUntil: 'networkidle' });
await page.evaluate(() => document.fonts.ready);
await page.waitForTimeout(1000);

// slides-ultra structure: count `.slide` elements inside `.deck`.
const slideCount = await page.evaluate(() => {
  const deck = document.querySelector('.deck');
  const scope = deck || document;
  return scope.querySelectorAll('.slide').length;
});
...
// ─── Screenshot each slide via ?preview=N (1-based) ───────

for (let i = 1; i <= slideCount; i++) {
  // Reload the deck in preview-only mode for slide i. The runtime renders just
  // this slide, chrome-free, and locks out navigation — exactly what we want.
  await page.goto(`${baseUrl}?preview=${i}`, { waitUntil: 'networkidle' });
  await page.evaluate(() => document.fonts.ready);
  await page.waitForTimeout(600);

  const screenshotPath = join(SCREENSHOT_DIR, `slide-${String(i).padStart(3, '0')}.png`);
  await page.screenshot({ path: screenshotPath, fullPage: false });
  screenshotPaths.push(screenshotPath);
  console.log(`  Captured slide ${i}/${slideCount}`);
}
...
# ─── Open the PDF automatically (best-effort, platform-specific) ──────
# `open` is macOS-only; `xdg-open` is Linux. On Windows/git-bash neither exists,
# so we fall back to `start` via cmd. If none are available we just skip — the
# PDF is already written, so this is purely a convenience step.
if command -v open &>/dev/null; then
    open "$OUTPUT_PDF"            # macOS
elif command -v xdg-open &>/dev/null; then
    xdg-open "$OUTPUT_PDF"        # Linux
elif command -v cmd.exe &>/dev/null; then
    # Windows / git-bash: hand the path to cmd's `start`. (Empty "" = window title.)
    cmd.exe /c start "" "$(cygpath -w "$OUTPUT_PDF" 2>/dev/null || echo "$OUTPUT_PDF")" 2>/dev/null || true
fi
