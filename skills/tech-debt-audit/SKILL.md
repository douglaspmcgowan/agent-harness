...
description: Thorough, user-invoked tech debt and architecture audit of the current codebase. Produces TECH_DEBT_AUDIT.md with file-cited findings, severity, effort estimates, and a required "looks bad but is actually fine" section. Use when the user asks for a debt audit, codebase health check, architecture review, or code quality assessment of an entire repo. Reserve it for explicit requests (a full audit is a 5-20 min pass) rather than firing it on any code mention.
---
...
> **Companion skill — `/declutter`.** This audit covers debt *broadly* (9 dimensions across the whole repo). For a *narrow, deep* pass on module DEPTH and interface design through one lens (Ousterhout's *A Philosophy of Software Design* + Matt Pocock's de-slop method) — finding shallow modules and proposing how to make them deep — use **`/declutter`**. Run this for breadth; run `/declutter` for structural depth. They compose: `/declutter` hands its breadth-of-debt findings here, and this audit's "Architectural decay" dimension (Phase 2 #1) is where `/declutter` goes deeper.
