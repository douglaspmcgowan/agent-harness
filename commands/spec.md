---
name: spec
description: "Produces the WHAT document for a build — one merged, technology-agnostic SPEC.md carrying three layers: §Product (the PRD content — root problem, users, jobs-to-be-done, measurable outcome metrics, non-goals), §Functional (prioritized independently-testable user stories P1/P2/P3 + FR-### requirements + key entities), and §Acceptance (a machine-readable ground-truth oracle — Given/When/Then scenarios each bound to a checkable grader, stable AC-### IDs, measurable-NFR thresholds). Grounded in IEEE 830 / ISO 29148 requirement quality, spec-kit's spec.md, EARS requirement shapes (Rolls-Royce, via Kiro), ATDD/BDD acceptance-as-executable-spec (Adzic), and Matt Pocock's to-spec (grill-first, synthesize-don't-re-interview, code-is-source-of-truth). The technical HOW (architecture, data model, contracts, tasks) is NOT in scope here — that stays with superpowers:writing-plans. Emits the doc the eval skills (/user, /probe, /design-review, /verify) read as their oracle. Invocable standalone (/spec <brief>) or via flags (--prd-only for just the §Product layer, --collaborate/--delegate for how ambiguity is resolved). Use when Douglas says 'write a spec', 'write the PRD', 'spec this out', 'what are the requirements for X', 'ground-truth doc for X', 'acceptance criteria for X', '/spec'."
---

# /spec [brief] [--prd-only] [--collaborate|--delegate]
...
- **Not `superpowers:writing-plans`.** writing-plans owns the HOW — architecture, data model, API contracts,
...
- **Not `superpowers:brainstorming`.** Brainstorming explores intent conversationally before creative work.
...
and any prior art (a path to an existing README/STATUS.md/design doc, or a `--claims` source). If the ask names
...
- Write `SPEC.md` to the **project root**, next to `STATUS.md`/`LOG.md` (Douglas's task-state convention). One
  file — `--prd-only` writes the same file with only §Product filled.
...
- **Doc-rot header.** The spec is a planning artifact, and the code is the source of truth once it ships
  ...
  whoever ships it marks the spec `superseded — see <review doc / STATUS.md>`, so a later agent never treats a
...
*Tracked copy: also save this file to `claude-global-config/commands/spec.md` (per the skills-are-tracked
