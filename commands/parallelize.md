...
**Principle:** each worker runs in isolated context — it never inherits your session history; you construct exactly what it needs. That keeps workers focused and preserves your own context for coordination. Dispatch one worker per independent problem domain. (This skill absorbs the former `/dispatching-parallel-agents`, archived in `commands/_merged/`.)
