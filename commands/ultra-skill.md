...
- **Not `/recon`.** `/recon` maps a landscape and helps Douglas DECIDE what to do next — most of its topics
  ...
  own auto mode executes whatever the chosen move actually is, generically. `/ultra-skill` starts from the
...
local surface the caller has already read this session** (a `/tsa`-style enqueue, a viewer build over a known
...
*Tracked copy: also save this file to `claude-global-config/commands/ultra-skill.md` (per the
