---
name: context-inbox-update
description: Scan the Context Inbox for new files and Random Dropbox entries. Research each one, create a note in Notes/, update _Map.md, reformat Random Dropbox.md, and archive processed raw files into organized subfolders. Run whenever you drop new material in the folder.
---

# /context-inbox-update

## Purpose

Detect anything new in the Context Inbox that isn't tracked yet, research it, document it in `Notes/`, keep `_Map.md` and `Random Dropbox.md` current, and move processed raw files into organized archive subfolders so the dropbox root stays clear for new drops.

Canonical folder: `C:\Users\<user>\Documents\<vault>\Context Inbox\`
...
```powershell
Get-ChildItem "C:\Users\<user>\Documents\<vault>\Context Inbox\" -File
```
...
File path: `C:\Users\<user>\Documents\<vault>\Context Inbox\Notes\<Descriptive Title>.md`
...
1. Search the vault for existing notes that mention the new note's subject — by person name, project name, tool name, or key term — but don't yet contain a wikilink to it.
   ```bash
   grep -ril "<key term>" "~/Documents/<vault>/" --include="*.md"
   ```
...
Drop anything here — links, names, tools, ideas, fragments. No format required. Run /context-inbox-update to process.
```
...
```bash
mv "~/Documents/<vault>/Context Inbox/<filename>" \
   "~/Documents/<vault>/Context Inbox/Archive/<subfolder>/<filename>"
```
...
Action needed:
  - Open Example Proposal 2023.docx → update stub
  - [other stubs]
...
- **No restricted data to external sources** — WebSearch/WebFetch are fine for public research about tools/people/concepts. Do not paste document contents into external search.
